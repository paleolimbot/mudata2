...
gc2data <- read_delim("data-raw/dunnington16a_alta_lake.txt", delim = "\t",
                      col_types = cols(
                        .default = col_double(),
                        zone = col_character(),
                        n_As = col_integer(),
...
# assign units
units <- c(As = "ppm", C = "%", Cu = "ppm", "d13C" = "per mille", 
           d15N = "per mille", K = "%", Mo = "ppm", Pb = "ppm",
           Rb = "ppm", SAR = "g/m2/yr", Ti = "%", Zn = "ppm")
gc2_long <- parallel_gather(gc2data, value = As:SAR, stdev = stdev_As:stdev_SAR, n = n_As:n_SAR,
                            key = "param") %>%
  mutate(units = units[param]) %>%
  select(age, depth, param, value, stdev, units, n, zone, everything())


alta_lake <- mudata(gc2_long, dataset_id = "alta_lake16", location_id = "ALGC2",
