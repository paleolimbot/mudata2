...
units <- c("LOI" = "%", "C" = "%", "N" = "%", "C/N" = NA_character_, 
...
geochem_long <- geochem %>%
  ...
  mutate(value = na_if(value, "ND") %>% as.numeric(),
         depth = as.numeric(depth)) %>%
  group_by(location_id, Unit, param, depth) %>%
  summarise(val = mean(value, na.rm = TRUE), sd = sd(value, na.rm = TRUE),
...
# put in bacon ages
bacon_ages <- read_delim("data-raw/long_lake/LL-PC1_43_ages.txt", delim = "\t",
                         col_types = cols(
                           depth = col_integer(),
                           min = col_double(),
                           max = col_double(),
...
data_long <- bind_rows(geochem_long, c14_ages_long, bacon_ages_long) %>%
  ungroup() %>%
  filter(param %in% c("bacon_age", "14C_age", "LOI", "C/N", "Ti", "Zr", "Fe/Mn",
