...
test_that("parallel_gather produces the expected output", {
  ...
  pocmaj_gathered <- parallel_gather(pocmajsum,
    key = "param",
    value = c(Ca, Ti, V),
...
test_that("parallel gather works wih tidyeval", {
  ...
  expect_identical(
    parallel_gather(pm2, key = "param", value = !!values, sd = !!sds),
    parallel_gather(pm2,
      key = "param",
      value = c(Ca, Ti, V), sd = c(Ca_sd, Ti_sd, V_sd)
    )
...
test_that("parallel gather throws errors", {
  ...
  # check unnamed args
  expect_error(
    parallel_gather(pocmajsum, key = "param", c(Ca, Ti, V)),
  ...
  expect_error(
    parallel_gather_base(pocmajsum,
      key = "param",
      list()
    ),
    "Must pass at least one value = columns in parallel_gather()"
