...
generate_type_tbl.mudata <- function(x, default = "guess") {
  ...
  allcols <- expand.grid(
    dataset = dataset_ids, table = names(x),
...
allowed_types_readr <- c(
  "date", "logical", "double", "character",
  "guess", "integer", "time"
)
...
parse_output_class <- function(type_str) {
  ...
  output_class_types <- c(
    "date" = "Date", "datetime" = "POSIXct", "logical" = "logical",
...
parse_json <- function(x, na = c("NA", ""), ...) {
  ...
  col <- parse_lapply(x, jsonlite::fromJSON,
    na = na, simplifyVector = TRUE,
...
parse_lapply <- function(x, fun, na = c("NA", ""), ...) {
  ...
  if (any(has_error)) {
    ...
    problems <- tibble::tibble(
      row = error_rows, col = NA_integer_,
...
parse_type_base <- function(type_str) {
  ...
  if (is.na(type_str) || (type_str == "")) {
    ...
    arg_names <- dplyr::coalesce(
      arg_match[, 2, drop = TRUE],
      arg_match[, 6, drop = TRUE],
      arg_match[, 9, drop = TRUE]
    )
    arg_values <- dplyr::coalesce(
      arg_match[, 4, drop = TRUE],
...
parse_list_string <- function(list_str) {
  ...
  item_values <- dplyr::coalesce(
    item_matches[, 3, drop = TRUE],
